...
describe("NameGenerator", () => {
  it("same seed produces same sector names", () => {
    const gen1 = new NameGenerator(42);
    const gen2 = new NameGenerator(42);
  ...
  it("generated sector names are non-empty strings", () => {
    const gen = new NameGenerator(99);
    for (let i = 0; i < 10; i++) {
  ...
  it("batch of 50 system names has no duplicates", () => {
    const gen = new NameGenerator(12345);
  ...
  it("batch of 50 mixed names has no duplicates", () => {
    const gen = new NameGenerator(77);
    const names: string[] = [];
  ...
  it("different seeds produce different names", () => {
    const gen1 = new NameGenerator(100);
    const gen2 = new NameGenerator(200);
    const name1 = gen1.generateSystemName();
    const name2 = gen2.generateSystemName();
    expect(name1).not.toBe(name2);
  });

  it("sector names contain a space (multi-word format)", () => {
